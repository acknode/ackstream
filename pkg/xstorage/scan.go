...
func UseScan(ctx context.Context, cfg *Configs, session *gocql.Session) (Scan, error) {
	...
	return func(sample *entities.Event, size int, page []byte) ([]entities.Event, []byte, error) {
		...
		ql := fmt.Sprintf("SELECT id, data, timestamps FROM %s WHERE bucket = ? AND workspace = ? AND app = ? AND type = ? ORDER BY id DESC", cfg.Table)
		...
		for scanner.Next() {
			...
			if err := scanner.Scan(&e.Id, &e.Data, &e.Timestamps); err != nil {
				iter.Close()
